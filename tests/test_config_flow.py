...
from custom_components.lockly import async_migrate_entry
...
async def test_config_flow_user(
    ...
    data = result["data"]
    assert data[CONF_FIRST_SLOT] == DEFAULT_FIRST_SLOT
    assert data[CONF_LAST_SLOT] == DEFAULT_LAST_SLOT
...
async def test_options_flow_updates_settings(
    ...
    hass.data["lockly_skip_frontend"] = True
    hass.data["lockly_skip_mqtt"] = True
    updated_first_slot = 3
    updated_last_slot = 10
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        {
            CONF_FIRST_SLOT: updated_first_slot,
            CONF_LAST_SLOT: updated_last_slot,
            CONF_MQTT_TOPIC: updated_topic,
            CONF_ENDPOINT: updated_endpoint,
        },
    )
    assert result["type"] == "create_entry"
    assert entry.options[CONF_FIRST_SLOT] == updated_first_slot
    assert entry.options[CONF_LAST_SLOT] == updated_last_slot
    assert entry.options[CONF_MQTT_TOPIC] == updated_topic
    assert entry.options[CONF_ENDPOINT] == updated_endpoint


@pytest.mark.enable_socket
async def test_migrate_entry_adds_slot_range(
    hass: HomeAssistant, enable_custom_integrations: Any
) -> None:
    """Test migration adds first/last slot defaults."""
    _ = enable_custom_integrations
    entry = MockConfigEntry(
        domain=DOMAIN,
        title="Lockly",
    ...
    assert entry.version == MIGRATION_VERSION
    assert entry.data[CONF_FIRST_SLOT] == DEFAULT_FIRST_SLOT
    assert entry.data[CONF_LAST_SLOT] == DEFAULT_MAX_SLOTS
